...
export const itineraryPreferencesService = {
    ...
    /**
 * Save file-only preferences (no form data validation)
 */

async saveFileOnlyPreferences(formData: IFrontendFormData): Promise<{
    success: boolean;
    data?: any;
    message?: string;
}> {
    try {
        ...
        // FIX: Properly extract file URLs from different possible locations
        const attachmentUrls: Record<string, { name: string; url: string }[]> = {};
        
        // Check uploadedFiles (from form data)
        if (formData.uploadedFiles && Object.keys(formData.uploadedFiles).length > 0) {
            Object.assign(attachmentUrls, formData.uploadedFiles);
        }
        
        // Check files array (alternative location)
        if (formData.files && Object.keys(formData.files).length > 0) {
            Object.assign(attachmentUrls, formData.files);
        }
        
        // Check metadata for files
        if (formData.metadata?.attachment_urls) {
            Object.assign(attachmentUrls, formData.metadata.attachment_urls);
        }

        // Validate we have files
        if (Object.keys(attachmentUrls).length === 0) {
            throw new Error('No files provided for file-only itinerary');
        }

        // Filter out blob URLs (only keep S3 URLs)
        const validAttachmentUrls: Record<string, { name: string; url: string }[]> = {};
        
        for (const [serviceType, files] of Object.entries(attachmentUrls)) {
            validAttachmentUrls[serviceType] = files.filter(file => 
                file.url && 
        ...
        console.log('Valid attachment URLs:', JSON.stringify(validAttachmentUrls, null, 2));

        // Create minimal itinerary with just files
        const fileData = {
            leadId: leadId,
            uploadedFiles: validAttachmentUrls,
            creationType: 'files-only',
            createdAt: new Date().toISOString()
        };

        const result = await itineraryPreferencesRepository.saveFileOnlyItinerary(fileData);
        ...
        console.error('Error saving file-only preferences:', error);
        return {
            success: false,
            message: error instanceof Error ? error.message : 'Failed to save file-only itinerary'
        };
    }
}
