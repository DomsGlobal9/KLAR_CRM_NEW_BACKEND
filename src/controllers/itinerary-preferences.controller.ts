        ...
        try {
            ...
            if (type === 'file') {
                const result = await withTimeout(
                    itineraryPreferencesRepository.getAllFileOnlyItineraries(paginationParams, roleFilter),
                    "getAllFileOnlyItineraries"
                );
                return res.status(200).json(result);
            }
            ...
            if (type === 'form') {
                const result = await withTimeout(
                    itineraryPreferencesRepository.getAllLeadsPaginated(paginationParams, roleFilter),
