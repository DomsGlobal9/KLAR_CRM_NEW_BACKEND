...
//         try {
//             const { id } = req.params;
//             const booking = await visaService.getVisaBookingByIdAndSource(id, 'B2B_PORTAL');
...
//             // Filters where source is NOT B2B_PORTAL (captures B2C configurations or user sessions)
//             const filter: any = { source: { $ne: 'B2B_PORTAL' } };

//             // If you track specific logged-in users, append email or user ID from your auth middleware context
//             if (email) {
...
//         try {
//             const { id } = req.params;
//             const booking = await visaService.getVisaBookingByIdAndSource(id, 'B2C');
//             res.status(200).json({ success: true, data: booking });
//         } catch (error) {
//             res.status(error instanceof Error && error.message === 'Visa booking not found' ? 404 : 500).json({
//                 success: false,
...
export class VisaController {
    ...
    async getB2CVisaBookingById(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params;
            // ✅ Fix: Sanitize parameter to resolve TS2345 mapping errors here as well
            const idStr = this.getStringParam(id);

            const booking = await visaService.getVisaBookingByIdAndSource(idStr, 'B2C');
            res.status(200).json({ success: true, data: booking });
        } catch (error) {
            res.status(error instanceof Error && error.message === 'Visa booking not found' ? 404 : 500).json({
                success: false,
